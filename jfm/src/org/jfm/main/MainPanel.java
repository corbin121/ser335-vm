...
public class MainPanel extends JPanel {
	...
	*/
	for (int i = 0; rpm != null && i < rpm.length; i++) {
	    ...
	    if (privileges.equals(CommonConstants.EDIT_ACTION)) {
	    	buttons.addElement(f4Button);
	    }
	    if (privileges.equals(CommonConstants.COPY_ACTION)) {
	    	buttons.addElement(f5Button);
	    }
	    if (privileges.equals(CommonConstants.MOVE_ACTION)) {
	    	buttons.addElement(f6Button);
	    }
	    if (privileges.equals(CommonConstants.MKDIR_ACTION)) {
